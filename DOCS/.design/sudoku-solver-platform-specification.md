...
| 4 | §7: structure validation mandatory; duplicate-constraint validation optional | The split is intentional: loaders validate shape/range; solver and API layers validate constraints. `DOCS/.architecture/validation-boundaries.md` (worklist item SUD-04, review Risk 4; adopted by DR-035) is authoritative on layer responsibilities; the optional strict loader mode is explicitly deferred there. |
