...
| 3 | §4: solver `getGrid` required operation | Implementations currently expose public working-grid members (`grid` / `Grid`) instead of a defensive `getGrid` snapshot. Restoring snapshot semantics is tracked as worklist item SUD-03 (review Risk 3); until it lands, the public member is the de facto read surface and direct external mutation is discouraged. |
