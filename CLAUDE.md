...
4. Stack-level docs under `DEMOAPPS/DEMOAPP001_TYPESCRIPT_CYPRESS/docs/`
5. This guide

`DECISION_REGISTER.md` is authoritative for structural and process decisions. Current accepted range: DR-001 through DR-014.

## Current Architecture Baseline

- Reference Architecture: v1.3, accepted 2026-05-15
- Active Stack: `DEMOAPP001_TYPESCRIPT_CYPRESS`
- Active surface: `@util` in-process class testing
- Canonical feature store: `features_shared/`
- Stack-local feature copy: `DEMOAPPS/DEMOAPP001_TYPESCRIPT_CYPRESS/tests/features/`
- Screenplay implementation: present and passing with Serenity/JS + Cucumber.js
- Known RA v1.3 gaps still tracked: MIG-04, MIG-05, MIG-08, MIG-09, MIG-10, MIG-11, MIG-12
...
|------------|----------|-----------|--------------|-------------|
| `DEMOAPP001_TYPESCRIPT_CYPRESS` | TypeScript 5.x | Cucumber.js + Serenity/JS | `@util` | `DEMOAPPS/DEMOAPP001_TYPESCRIPT_CYPRESS/` |
...
Run Stack commands from `DEMOAPPS/DEMOAPP001_TYPESCRIPT_CYPRESS/`.
...
Available puzzle data is in `DEMOAPPS/DEMOAPP001_TYPESCRIPT_CYPRESS/puzzles.json`.

## Canonical Feature Update Procedure

When adding or changing Gherkin behavior:

1. Edit the canonical feature first: `features_shared/util-tests/sudoku-solver/BasicSudokuSolverLogic.feature`.
2. Copy the changed feature body to `DEMOAPPS/DEMOAPP001_TYPESCRIPT_CYPRESS/tests/features/BasicSudokuSolverLogic.feature`.
...
```text
DEMOAPPS/DEMOAPP001_TYPESCRIPT_CYPRESS/tests/screenplay/step_definitions/
```
...
Current known Screenplay compliance gaps:

- MIG-04: runtime state is still mainly held on Ability fields rather than Actor Memory.
- MIG-05: some step definitions still call Abilities directly instead of delegating only through Tasks and Questions.

Do not describe the Screenplay layer as absent. It exists and passes; the remaining work is contract tightening.

## Parity Rules Summary

Governed by `DOCS/REFERENCE_ARCHITECTURE.md` v1.3 and decisions DR-012 through DR-014.

| Rule | Current instruction |
|------|---------------------|
| Decision authority | `DECISION_REGISTER.md` wins over restatements elsewhere |
| Feature source | `features_shared/` is canonical; Stack features are copies with stack tags |
...
|------|------|---------------|
| Feature drift | `features_shared/` and Stack `tests/features/` | Canonical body and Stack-local body should match after local tag differences |
...
3. Add canonical feature coverage in `features_shared/`.
