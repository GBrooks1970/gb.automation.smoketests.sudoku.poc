...
- BACKLOG-064 (SUD-25, review `CODE_REVIEW_CODEX_v1_20260723T2351Z` Risk 4, MEDIUM):
  ...
  coverage.py branch collection under Python 3.13 CI and documented its selected-module baseline
  and exclusions.
...
- SUD-06 (review `CODE_REVIEW_GPT_5_3_Codex_v1_20260530T0823Z` Risk 6, Low): documented (rather than code-hardened — the lower-risk of the two offered options) DEMOAPP003's integer-validation mechanism. The DEMOAPP003 C# README gains a "Puzzle Validation" section stating that typed deserialization into `Puzzle.Grid` (`int[][]`) is the integer-type check: `System.Text.Json` rejects non-integer JSON tokens at load time (surfacing as a `JsonException`) before the v1.0 §7.1 range check runs on the typed integers. This is documented as a mechanism difference, not a responsibility difference, from DEMOAPP001 (`Number.isInteger`) and DEMOAPP002 (`isinstance(cell, int)`). `DOCS/.architecture/validation-boundaries.md` §2.1 updated to record this resolution and the deliberate decision not to wrap the exception into the v1.0 wording. Docs-only; no C# code or behaviour change.
...
- BACKLOG-064: the Python 3.13 test lock now includes coverage.py 7.15.2; CI emits a report-only
  ...
  with no threshold until SUD-28 reviews exclusions and mutation evidence.
