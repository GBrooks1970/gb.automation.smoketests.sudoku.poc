// app.js (static evidence build — BACKLOG-071 / DR-040).
//
// The same orchestration as the API-served viewer, but it reads PRECOMPUTED
// solve payloads from relative static files (./data/...) instead of calling the
// REST service. No server, no live endpoints — suitable for GitHub Pages. The
// grid and player modules are the maintained, unmodified DEMOAPP001 modules.

import { createGrid, renderGridAtStep } from './grid.js';
import {
  load as playerLoad,
  goFirst, goPrev, goNext, goLast, togglePlay, goTo, setSpeed,
} from './player.js';

// ── App state ───────────────────────────────────────────────
let solveData    = null;   // VisualiseResult from a precomputed fixture
let originalClues = null;  // number[][] — cells pre-filled in the puzzle
...
// ── Solve + visualise (static fixture) ──────────────────────
async function loadSolveData(slug) {
  resetUI();
  showLoading(true);

  try {
    const res = await fetch(`./data/visualise/${encodeURIComponent(slug)}.json`);
    if (!res.ok) {
      throw new Error(`No precomputed data for "${slug}" (${res.status})`);
    }
