...
export const GridCell = {
  at: (row: number, col: number) =>
    Question.about(
      `grid cell [${row},${col}]`,
      (actor) => UseSudokuSolver.as(actor).getSolver().grid[row][col]
    ),

  containsValue: (value: number) =>
    Question.about(`grid contains ${value}`, (actor) =>
      UseSudokuSolver.as(actor)
        .getSolver()
        .grid.some((r) => r.includes(value))
    ),
  ...
  allFilled: () =>
    Question.about('all cells filled', (actor) =>
      UseSudokuSolver.as(actor)
        .getSolver()
        .grid.every((r) => r.every((c) => c !== EMPTY_CELL))
    ),
  ...
  workingDiffersFromOrig: () =>
    Question.about('working grid differs from origGrid', (actor) => {
      ...
      return solver.grid.some((row, r) => row.some((cell, c) => cell !== solver.origGrid[r][c]));
    }),
  ...
  hasEmptyCells: () =>
    Question.about('grid has empty cells', (actor) =>
      UseSudokuSolver.as(actor)
        .getSolver()
        .grid.some((r) => r.some((c) => c === EMPTY_CELL))
    ),

  isGridFull: () =>
    Question.about('grid is full', (actor) => UseSudokuSolver.as(actor).isGridFull()),
  ...
  isDeepCopy: () =>
    Question.about(
      'working grid is a deep copy of the snapshot (same values, different reference)',
      (actor) => {
        const ability = UseSudokuSolver.as(actor);
        const grid = ability.getSolver().grid;
        const snapshot = ability.gridSnapshot;
