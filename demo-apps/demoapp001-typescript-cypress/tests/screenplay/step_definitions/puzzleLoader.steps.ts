...
Given('a puzzle with an 8x9 grid in the JSON file', async () => {
  await actorCalled(SOLVER_ACTOR).attemptsTo(SimulateError.forInvalidRowCount(8));
});
...
When('requesting a puzzle by name {string}', async (name: string) => {
  await actorCalled(SOLVER_ACTOR).attemptsTo(LoadPuzzleByName.andInitialiseOrDefault(name));
});

When('requesting puzzles with difficulty {string}', async (difficulty: string) => {
  await actorCalled(SOLVER_ACTOR).attemptsTo(LoadPuzzlesByDifficulty.andStore(difficulty));
});

When('requesting puzzle at index {int}', async (index: number) => {
  await actorCalled(SOLVER_ACTOR).attemptsTo(LoadPuzzleByIndex.andInitialise(index));
});

When('the PuzzleLoader is initialized', async () => {
  // For the "file does not exist" scenario, simulate the missing file error
  await actorCalled(SOLVER_ACTOR).attemptsTo(SimulateError.forMissingFile());
});
...
Then('{int} puzzles should be successfully loaded', async (count: number) => {
  const actual = await actorCalled(SOLVER_ACTOR).answer(LoadedPuzzleCount.current());
  assert.strictEqual(actual, count);
});

Then('each puzzle should have a name, difficulty, description, and grid', async () => {
  const puzzles = await actorCalled(SOLVER_ACTOR).answer(LoadedPuzzles.all());
...
Then('the correct puzzle should be returned', async () => {
  const name = await actorCalled(SOLVER_ACTOR).answer(CurrentSolver.name());
  assert.strictEqual(name, 'Easy Scan Grid');
});

Then('the puzzle grid should be a 9x9 array', async () => {
  const valid = await actorCalled(SOLVER_ACTOR).answer(CurrentSolver.hasValidGrid());
