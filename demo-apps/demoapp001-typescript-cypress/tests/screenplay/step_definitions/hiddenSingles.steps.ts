...
Given('row {int} is missing the number {int}', async (rowIndex: number, target: number) => {
  await actorCalled(SOLVER_ACTOR).attemptsTo(SetupGridState.rowMissingDigit(rowIndex, target));
});

Given(
  '{int} cells in row {int} cannot contain {int} due to column or block constraints',
...
Given('column {int} is missing the number {int}', async (colIndex: number, target: number) => {
  await actorCalled(SOLVER_ACTOR).attemptsTo(SetupGridState.columnMissingDigit(colIndex, target));
});

Given(
  '{int} cells in column {int} cannot contain {int} due to row or block constraints',
...
Given(
  'the number {int} is present in rows that intersect three of those empty cells',
  async (target: number) => {
    await actorCalled(SOLVER_ACTOR).attemptsTo(SetupGridState.hiddenSingleInBlock(target));
  }
);

Given('row {int} already contains the digit {int}', async (rowIndex: number, digit: number) => {
  await actorCalled(SOLVER_ACTOR).attemptsTo(SetupGridState.digitInRow(rowIndex, digit));
});

Given(
  'the number {int} can be placed in multiple locations within a unit',
...
When(
  'the {string} algorithm is executed for value {int}',
