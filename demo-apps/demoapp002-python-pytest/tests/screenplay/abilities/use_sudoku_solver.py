...
class UseSudokuSolver:
    def __init__(self):
        ...
        # SUD-22 / BACKLOG-061: attempt evidence is independent from the opt-in audit response.
        # A plain solve can prove unchanged invocations while last_audit_trail remains None.
        self._last_attempt_events: tuple[AttemptEvent, ...] = ()
    ...
    def apply_x_wing(self) -> None:
        self._last_algorithm_changed = self.get_solver().x_wing()
    ...
    def solve_puzzle(self) -> None:
        ...
        self._last_attempt_events = tuple(events)
        self._last_audit_trail = None

    def solve_puzzle_with_audit(self) -> None:
        events: list[AttemptEvent] = []
        orchestrator = SudokuOrchestrator(
            self.get_solver(), {"enabled": True}, attempt_observer=events.append
        )
        self._solve_result = orchestrator.solve()
        self._last_attempt_events = tuple(events)
    ...
    @property
    def last_attempt_events(self) -> tuple[AttemptEvent, ...]:
        return self._last_attempt_events

    @property
    def last_attempt_iterations(self) -> int:
        return len({event.iteration for event in self._last_attempt_events})
